...
function imageFocus(focus){
	for(var i = 0; i < imageCount; i++){
		if($imageBank[i].attr('src') == $(focus).attr('src')){
			...
			//$("#gallery-main iframe").attr("src",$(focus).attr('data-src'));
			setTimeout(function () {
				$("#gallery-main iframe").attr("src",$(focus).attr('data-src'));
...
function generateThumbs(){
	...
	for(var i = 0; i < imageCount; i++){
		...
		var $tempObj = $('<img id="'+i+'t" class="thumb" src="'+$imageBank[i].attr('src')+'"  data-src="'+$imageBank[i].attr('data-src')+'" />');

		if(i == 0)
			$tempObj.addClass("selected");

		$("#thumbcon").append($tempObj);
