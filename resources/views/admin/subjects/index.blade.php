			...
			<div class="card">
				<div class="card-header">
					<div class="row">
						...
						<div class="col-md-2">
							<a href="{{ route('admin.subjects.create') }}" class="btn btn-primary btn-sm custom_btn float-right"><i class="fa fa-plus" aria-hidden="true"></i>
                            Add New</a>
						</div>
					</div>
				</div>
                <div class="card-body table-border-style mb-2">
				    <!-- Filter Form -->
                    <form action="{{ route('admin.subjects.index') }}" method="GET" id="filter-form">
                        <div class="row mb-4">
                            <div class="col-md-4">
                            ...
                            <div class="col-md-3">
                                <select name="course_id" class="form-control">
									...
									@foreach ($courses as $id => $title)
										<option value="{{ $id }}" @selected(old('course_id', request()->input('course_id')) == $id)>
											{{ $title }}
										</option>
									@endforeach
								</select>
                            ...
                            <div class="col-md-3 fiter-btn-pd">
                                ...
                                <a href="{{ route('admin.subjects.index') }}" class="btn btn-dark btn-sm reset-btn">
					...
					<div class="table-responsive">
						<table class="table table-hover">
							...
							<tbody>
								...
								@else
									@php
										$sNo = ($subjects->currentPage() - 1) * $subjects->perPage() + 1;  /* Increasing Serial Number */
									@endphp
									...
									<tr>
										...
										<td>{{ ($subject->status) ? 'Active' : 'In-active' }}</td>
										<td>{{ $subject->created_at }}</td>
										<td>{{ $subject->updated_at }}</td>
										<td class="noselect text-right">	
											<div class="action-tools">
												<a href="{{ route('admin.subjects.edit', $subject) }}" class="btn btn-info btn-sm action-btn edit" data-toggle="tooltip" title="" data-original-title="Edit">
													<i class="far fa-edit"> Edit</i>
												</a>	
												
												<form id="{{ 'SubjectDelete_'.$subject->id }}" action="{{ route('admin.subjects.destroy', $subject->id) }}" method="POST" style="display: inline;"  onsubmit="confirmation(event)">
													@csrf
													@method('DELETE')
