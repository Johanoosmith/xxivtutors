	...
	<div class="container">
		<div class="row">
			...
			<div class="col dashboard-content">
				...
				<!-- Filter Form -->
				<form action="{{ route('customer.booking.index') }}" method="GET" id="filter-form">
					<div class="row mb-4">
						<div class="col-md-3">
					...
					<table class="table table-striped">
						<thead>
							<tr>
								...
								<th>Lesson</th>
								<th>Subject</th>
								<th>Tutor Rate ({{ config('constants.CURRENCY_SYMBOL') }})</th>
								<th>Status</th>
						...
						<tbody>
							...
							<tr>
								...
								<td>
									{{ date('d/m/Y', strtotime($booking->start_date)) .' at '. date('H:i', strtotime($booking->start_time)) }}
									<br>
									{{ $booking->duration . ' Minutes' }}
								</td>
								<td>{{ $booking->subject->title }} <strong>( {{$booking->level->title}} )</strong></td>
								<td>{{ $booking->student_rate }}</td>
								<td>
